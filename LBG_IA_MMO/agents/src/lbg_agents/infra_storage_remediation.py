...
def build_storage_remediation_plan(
    ...
    *,
    ...
    vm246 = str(storage.get("vm246_status") or "")
    vg_free = str(storage.get("vg_free") or "")
    hints: list[str] = []
    actions: list[dict[str, Any]] = []

    if outcome == "ok" and "io-error" not in vm246:
        return {
            "kind": "remediation_plan",
            "source": "proxmox_storage",
            "outcome": "ok",
            "data_percent": data_pct,
            "hints": [f"Pool thin OK ({data_pct}%)"],
            "suggested_actions": [],
        }
    ...
    if "io-error" in vm246:
        ...
        actions.append(
            {
                "level": "manual",
                "label": "Proxmox : lvextend -l +100%FREE pve/data puis qm stop/start 246",
    ...
    if outcome in {"warn", "critical"}:
        ...
        if vg_free and vg_free not in {"0", "0.00g", "<0"}:
            actions.append(
                {
                    "level": "manual",
                    "label": f"Étendre le pool thin (VG libre {vg_free})",
