"""Tests sonde stockage Proxmox (pool thin local-lvm)."""
...
def test_probe_warn_outcome(monkeypatch):
    ...
    with patch("lbg_agents.proxmox_storage_probe._ssh_proxmox", return_value=(True, stdout, "")):
...
def test_probe_ssh_failure():
    with patch("lbg_agents.proxmox_storage_probe._ssh_proxmox", return_value=(False, "", "timeout")):
        payload = probe_proxmox_storage_local()
