...
push_one() {
  ...
  ssh -tt "${SSH_OPTS[@]}" "${VM_USER}@${VM_HOST}" "bash -lc '
    ...
    if [ \"${RESTART}\" = \"1\" ]; then
      ...
      for svc in lbg-agent-dialogue lbg-agent-quests lbg-agent-combat lbg-agent-pm lbg-orchestrator lbg-backend lbg-mmo-server lbg-mmmorpg-ws lbg-core3-ia-sidecar lbg-openclaw-bridge; do
