...
|------|---------------------|
| 2026-05-12 | **new_mmo / Core3** : ADR **`docs/adr/0005-new-mmo-core3-coexistence.md`** (coexistence avec `mmmorpg_server` / `mmo_server`, pas de remplacement implicite) ; guide **`docs/migration_new_mmo_core3.md`** ; `third_party/README.md` ; ignore Git **`third_party/new_mmo/`** ; tableau des dépôts dans **`plan_fusion_lbg_ia.md`** ; **`architecture.md`** — lien ADR 0005. |
| 2026-05-07 | **Agent hybride proactif (module autonome)** : package `hybrid_proactive_agent/` (installable, **hors orchestrateur** pour limiter le couplage) — moteur 3 modes, mémoire JSONL, coordinateur multi-rôles ; doc `hybrid_proactive_agent/README.md` + `hybrid_proactive_agent/docs/*` ; tests `hybrid_proactive_agent/tests/` ; port navigateur optionnel `web_client/src/lib/hybridProactiveEngine.ts`. |
| 2026-05-07 | **Agent hybride — greffon (doc)** : `hybrid_proactive_agent/docs/GREFFON.md` enrichi (matérialisation : dépendance venv, état par session, persistance optionnelle, communication via `output` orchestrateur / Pilot, évolution vers `action_proposal` et policy sans exécution directe par le moteur). |
| 2026-05-07 | **Plan bot compagnon autonome** : microservice + chat naturel (JSON réservé au debug), mémoire persistante, UI type Vite (LAN), phasage jusqu’à usage orchestrateur comme client — **`docs/plan_bot_compagnon_autonome.md`**. |
| 2026-05-07 | **Companion bot — Phase 1 (implémentation)** : ajout du microservice `companion_bot/` (`POST /v1/chat`, `GET /healthz`, persistance SQLite, adaptateur LLM OpenAI-compatible + stub), tests `companion_bot/tests/*`, env `LBG_COMPANION_*` dans `infra/secrets/lbg.env.example`, unité systemd `infra/systemd/lbg-companion-bot.service`. |
| 2026-05-07 | **Companion bot — Phase 2 (UI + déploiement front)** : UI Vite/React `companion_bot/web` (debug masqué `?debug=1`), poll `events`, script `infra/scripts/deploy_companion_web.sh` (build `--base=/compagnon/` + copie vers `pilot_web/compagnon/` + restart Nginx), et Nginx `infra/nginx/pilot_web_110.conf.example` route `/compagnon/`. |
...
| 2026-05-05 | **Mobs** : `mob|monster` ne déclenchent plus le pont IA (pas de dialogue “courtois”) ; côté client, pas d’envoi de dialogue vers un mob, et pas de sélection/attaque d’une cible déjà vaincue (HP ≤ 0). |
| 2026-05-05 | **UI client MMO** : HUD réorganisé (barre bas chat+fiche PNJ, wrap responsive) + panneaux hauts scrollables (moins de superpositions). |
| 2026-05-05 | **Collisions** : suppression de la soustraction automatique `padding_tiles` côté rendu (évite décalage fond ↔ grille) ; côté WS, désactivation des obstacles “boîtes seed” quand la grille Watabou est chargée (évite les murs invisibles). |
| 2026-05-05 | **MMO PNJ** : lissage vitesse / orientation (`NPC_STEER_VEL_TAU_S`, `_sm_vx`/`_sm_z` dans `stats`, `NPC_STEER_YAW_MAX_RAD_S`) ; amorti **`vx`/`vz` ×0.6** réservé aux **joueurs** (plus de battement contre le steering PNJ). |
| 2026-05-05 | **Patrouille garde** : jalons hors seed (`porte_nord` / ±180 m) ramenaient tous les buts au même snap walkable → quasi immobile même sans dialogue ; ronde sur POIs du seed (**`hotel_de_ville`**, **`marche`**, **`forge`**, **`auberge_salle_commune`**) quand présents. **Bug arrivée** : le test `dist < 3` s’appliquait au pas A* (≈2 m tuile) au lieu du lieu cible — la ronde avançait sans marche. Correction : `dist_goal` vs `goal_*`, marche vers `step_*`. Pendant `busy_timer`, `wait_timer` via `_npc_step(..., timers_only=True)`. Fichiers `mmmorpg_server/game_state.py` ; tests `tests/test_npc_conversation_pause.py`, `tests/test_guard_patrol.py`. |
...
| 2026-05-05 | **MMO WS — reconnexion + anti‑spam + PNJ “walkable”** : `welcome.session_token` + `hello.resume_token` (reprise `player_id`, GC après `MMMORPG_SESSION_TTL_S`) ; erreur `rate_limited: move` (en plus du drop) ; interactions inventaire `player_item_*` bornées par distance (`MMMORPG_ITEM_INTERACT_MAX_DISTANCE_M`) ; pathfinding A* (pas de tuile) utilisé par gardes + errance PNJ (grille `watabou_grid_v1`). Doc : `mmmorpg_server/README.md`, `web_client/README.md`. |
