...
- `GET /v1/world/collision-grid` — export JSON **`watabou_grid_v1`** complet pour le **client** (prédiction de mouvement alignée sur l’autorité). Pour le navigateur, activer **`LBG_MMO_CORS_ORIGINS`** sur le `mmo_server`, ou en dev local **`LBG_MMO_CORS_DEV=1`** si la liste est vide (voir `mmo_server/README.md`).

### Serveur WebSocket `mmmorpg_server`

Les déplacements côté autorité WS utilisent la même convention (`.` / `R` franchissables). Chargement du JSON `watabou_grid_v1` via `MMMORPG_VILLAGE_GRID_JSON`, puis `LBG_MMO_VILLAGE_GRID_JSON`, puis le même fichier seed `pixie_seat.grid.json` que `mmo_server` si les variables ne pointent pas vers un autre fichier. Si le point monde `(0, 0)` n’est pas sur une tuile franchissable, le **spawn joueur** utilise la première tuile franchissable trouvée en **spirale** depuis la tuile sous `(0, 0)` (centre de tuile en coordonnées monde). Les **PNJ** issus du seed sont recalés sur la tuile franchissable la plus proche si une grille est active ; à chaque tick, **joueurs et PNJ** partagent la même vérification (tuile + obstacles seed) avant déplacement. Le **client web** (`web_client`) peut charger la même grille via `GET /v1/world/collision-grid` sur le `mmo_server` (CORS `LBG_MMO_CORS_ORIGINS`) pour une prédiction de mouvement locale alignée. Voir `mmmorpg_server/README.md` (section collisions village).

### Seed Pixie Seat — POI bâtiments, maisons PNJ, fonds auberge / forge

- **`mmo_server/world/seed_data/world_initial.json`** : chaque bâtiment du layout Watabou peut être exposé comme **location** (`id` du type `building:b_XXXX` ou POI nommés `auberge_salle_commune`, `forge`, `hotel_de_ville`, `marche`, …). Les **petits bâtiments** servent de **maisons** : `type: "house"` et `situation.location` des PNJ civils pointe vers l’`id` du bâtiment choisi.
- **Orientation** : pour l’auberge et la forge, le polygone Watabou du bâtiment donne un angle (côté le plus long) ; il est stocké en **`geometry.rotation_rad`** dans le seed. Le **`mmmorpg_server`** transmet ce champ dans les `locations` du tick WS ; le **client** (`web_client/src/renderer.js`) applique cette rotation au dessin des sprites **plancher auberge** / **plancher forge** pour aligner visuellement avec l’emprise métrique (largeur × hauteur du AABB), sans réinventer une échelle hors grille.
- **État Lyra côté `mmo_server`** : `Npc` inclut `situation` et `goals` ; `WorldState` inclut `locations` (et `get_location_by_tag` pour la taverne dans le moteur social). La persistance JSON sérialise désormais **locations** + champs PNJ utiles au comportement LOD — à garder cohérent avec le seed.

**Anti-piège (prochain village)** : la **carte PNG décorative** peut être mise à l’échelle / retournée (`pflipz`, `ps` dans le client, voir `web_client/README.md`) pour caler sur la **grille `watabou_grid_v1`** ; les **dimensions POI** viennent du **layout / seed**, pas du PNG seul.
