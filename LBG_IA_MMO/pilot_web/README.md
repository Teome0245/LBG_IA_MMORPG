...
- **Assistant Core** `#/assistant` : flux texte naturel → `POST /v1/pilot/action-proposal` (proxy vers orchestrateur) → `ActionProposal` éditable → application dans `context` → `POST /v1/pilot/route` en dry-run ou réel avec approval. Affiche le résultat brut et lisible, dont `output.policy`.
- **Assistant Core — mémoire légère** : `#/assistant` stocke uniquement dans le navigateur un historique court, des préférences explicites (chemin Notepad, dry-run par défaut) et des suggestions non destructives après proposition/exécution. **Résumé de session volontaire** : agrégat JSON (notes + historique) pour copie locale — pas d’envoi serveur implicite.
