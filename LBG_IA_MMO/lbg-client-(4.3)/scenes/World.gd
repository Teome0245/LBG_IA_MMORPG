...
var _entity_packed: PackedScene
...
func _ready() -> void:
	add_to_group("world")
	_entity_packed = load(ENTITY_SCENE_PATH) as PackedScene
	if _entity_packed == null:
		push_error("EntityView introuvable: %s" % ENTITY_SCENE_PATH)
		_append_chat("ERR", "EntityView.tscn — rechargez le projet dans Godot")
	...
	_append_chat("SYS", "Clic sol=déplacer | Clic PNJ=cible | Clic droit=regarder | Molette=zoom | ZQSD")
...
func _cycle_target() -> void:
	...
	if not tgt.is_empty():
		_append_chat("SYS", "Cible: %s" % str(tgt.get("name", GameState.target_npc_id)))
	...
	if event is InputEventMouseButton:
		...
		if mb.pressed and mb.button_index == MOUSE_BUTTON_LEFT and not _chat_input.has_focus():
			...
			if not picked.is_empty():
				...
				_update_hud()
				var tgt := GameState.target_npc_snapshot()
				_append_chat("SYS", "Cible: %s" % str(tgt.get("name", picked)))
			else:
...
func _handle_npc_reply(msg: Dictionary) -> void:
	...
	var tgt := GameState.target_npc_snapshot()
	var name := str(tgt.get("name", "PNJ"))
	_append_chat(name, shown)

func _refresh_all_entities() -> void:
	var player_snap := GameState.local_player_snapshot()
	if not player_snap.is_empty() and not _predict_local:
	...
	for eid in GameState.entities:
		var ent: Dictionary = GameState.entities[eid]
		# Un seul avatar joueur : le nôtre (les doublons même pseudo sont filtrés dans GameState)
		if str(ent.get("kind", "")) == "player" and eid != GameState.player_id:
			continue
		if not _views.has(eid):
			if _entity_packed == null:
				continue
			var node: Node3D = _entity_packed.instantiate() as Node3D
			_entities_root.add_child(node)
			if node.has_method("setup"):
				node.call("setup", ent)
			_views[eid] = node
		else:
			var v: Node3D = _views[eid]
			if v.has_method("update_from_snapshot"):
				v.call("update_from_snapshot", ent)
	_sync_local_view()
	_refresh_target_highlight()
	_update_hud()
...
func _refresh_target_highlight() -> void:
	for eid in _views:
		var v: Node3D = _views[eid]
		if v.has_method("set_targeted"):
			v.call("set_targeted", eid == GameState.target_npc_id)
